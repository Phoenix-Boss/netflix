...
async def _fetch_tmdb_meta(dbid: str, mtype: str) -> dict:
    ...
    if not api_key: raise HTTPException(status_code=500, detail="TMDB_API_KEY not configured on server.")
    url = f"https://api.themoviedb.org/3/{mtype}/{dbid}?api_key={api_key}&append_to_response=credits"
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(url, timeout=10.0)
            if resp.status_code != 200: raise HTTPException(status_code=404, detail="TMDB metadata not found.")
            d = resp.json()
            certification = ""
            if mtype == "movie":
                try:
                    dates = d.get("release_dates", {}).get("results", [])
                    if dates: certification = dates[0].get("release_dates", [{}])[0].get("certification", "")
                except: pass
            else:
                try:
            ...
            return {"id": d.get("id"), "title": d.get("title") or d.get("name"), "overview": d.get("overview", ""), "poster_path": d.get("poster_path", ""), "backdrop": d.get("backdrop_path", ""), "rating": d.get("vote_average", 0), "vote_count": d.get("vote_count", 0), "runtime": d.get("runtime") or (d.get("episode_run_time", [None])[0]), "release_date": d.get("release_date") or d.get("first_air_date", ""), "genres": [g["name"] for g in d.get("genres", [])], "number_of_seasons": d.get("number_of_seasons", 0), "status": d.get("status", ""), "tagline": d.get("tagline", ""), "popularity": d.get("popularity", 0), "budget": d.get("budget", 0), "revenue": d.get("revenue", 0), "certification": certification, "cast": [{"name": c["name"], "character": c.get("character", ""), "profile_path": c.get("profile_path")} for c in d.get("credits", {}).get("cast", [])[:15]]}
        except httpx.TimeoutException: raise HTTPException(status_code=504, detail="TMDB API timeout")
        except Exception as e: raise HTTPException(status_code=500, detail=f"TMDB API error: {str(e)}")

@app.api_route("/", methods=["GET", "HEAD"])
async def index():
    try: return await info()
    except Exception as e: return {"status": "ok", "message": "Streaming API is running", "version": VERSION, "python_version": sys.version.split()[0]}

@app.get("/health")
@app.head("/health")
async def health():
    return {"status": "healthy", "version": VERSION, "python_version": sys.version.split()[0], "providers": ["vidapi", "fzmovies", "o2tv", "kissasian", "dramacool", "shortdrama", "torrents"]}

@app.get("/rcp/{dbid}")
async def get_rcp(dbid: str, type: str = "movie"):
    try: return {"status": 200, "meta": await _fetch_tmdb_meta(dbid, type)}
...
async def get_prorcp(dbid: str, type: str = "movie", s: int = None, e: int = None, title: str = None):
    ...
    try:
        meta_task, stream_task = _fetch_tmdb_meta(dbid, type), extract(dbid, s, e, title=title)
        ...
        if isinstance(meta_res, Exception): raise HTTPException(status_code=404, detail=f"Meta failed: {str(meta_res)}")
        if isinstance(stream_res, Exception): stream_res = None
        subs = []
        if stream_res:
            try: subs = await get_subtitles(stream_res.get("imdb_id", ""), stream_res.get("title") or meta_res.get("title"), mt, s, e)
            except: pass
        return {"status": 200, "meta": meta_res, "sources": format_sources(stream_res, subs) if stream_res else [], "stream_url": stream_res.get("stream_urls", [None])[0] if stream_res and stream_res.get("stream_urls") else None, "provider": stream_res.get("provider") if stream_res else None}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
async def asian_stream(title: str, s: int = None, e: int = None, provider: str = "dramacool"):
    ...
    try:
        if provider.lower() == "kissasian":
            ...
            if not result or not result.get("url"): raise HTTPException(status_code=404, detail="Not found on KissAsian.")
            ...
            if not result or not result.get("url"): raise HTTPException(status_code=404, detail="Not found on DramaCool.")
...
async def shortdrama_stream(title: str, site: str = "reelshort", episode: int = None):
    try:
        ...
        if not result or not result.get("url"): raise HTTPException(status_code=404, detail="Short drama not found.")
...
async def torrent_stream(dbid: str, s: int = None, e: int = None, title: str = None, quality: str = "1080p"):
    if not dbid and not title: raise HTTPException(status_code=400, detail="ID or title is required")
    try:
        ...
        if not result or not result.get("_torrent_data", {}).get("magnet"): raise HTTPException(status_code=404, detail="No torrents found")
        mt = "tv" if s is not None and e is not None else "movie"
        subs = []
        try: subs = await get_subtitles(result.get("imdb_id", ""), result.get("title", ""), mt, s, e)
        except: pass
        return {"status": 200, "info": "success", "provider": result.get("provider", "Torrent"), "torrent": result["_torrent_data"], "magnets": get_all_magnets(result), "sources": format_torrent_sources(result, subs)}
    except HTTPException: raise
...
async def torrent_search_endpoint(title: str, quality: str = "1080p"):
    ...
    try:
        ...
        if not result or not result.get("_torrent_data", {}).get("magnet"): raise HTTPException(status_code=404, detail="No torrents found")
        return {"status": 200, "info": "success", "torrent": result["_torrent_data"], "alternatives": result.get("alternatives", []), "magnets": get_all_magnets(result)}
    except HTTPException: raise
    except Exception as ex: raise HTTPException(status_code=500, detail=str(ex))
        ...
        if q in ["480p", "720p"] and s is not None and e is not None:
            ...
            if primary_result and primary_result.get("title"):
                ...
                if o2tv_result and o2tv_result.get("download_url"):
                    ...
                    return {"status": 200, "provider": "o2tv (Direct MP4)", "sources": o2tv_format(o2tv_result, subs, needs_transcode=False)}
                else:
                    subs = await get_subtitles(primary_result.get("imdb_id", ""), title, "tv", s, e)
...
async def test_proxy():
    try:
        ...
        except Exception as e: results["direct_https"] = {"error": str(e)}
...
async def subs(url: str):
    ...
    except Exception as e: raise HTTPException(status_code=500, detail=f"Error fetching subtitle: {str(e)}")

@app.get("/cache/stats")
async def get_cache_stats(): return cache_stats()
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={"status": exc.status_code, "detail": exc.detail})
